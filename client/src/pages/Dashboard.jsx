      ...
      <div className="col-span-12 lg:col-span-8 space-y-6">
        ...
        <section className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
          <div className="px-4 py-3 border-b border-gray-100 font-semibold text-gray-800 flex justify-between items-center">
            ...
            <button 
              ...
              className={`text-xs px-2 py-1 rounded flex items-center gap-1 border transition-colors
                ${isVideoOn ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100' : 'bg-gray-100 text-gray-600 border-gray-200 hover:bg-gray-200'}`}
            >
          ...
          <div className="aspect-video bg-black relative flex items-center justify-center group overflow-hidden">
            {isVideoOn ? (
              <>
                 <div className="absolute inset-0 bg-neutral-900 flex flex-col items-center justify-center text-white/30">
                    <Video size={48} className="mb-2 animate-pulse" />
                    <span className="text-sm">📷 Robot Camera Feed (Simulation)</span>
                 </div>
              ...
              <div className="text-gray-500 flex flex-col items-center gap-2">
                <VideoOff size={32} />
                <span className="text-sm">카메라 연결이 중지되었습니다.</span>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm p-5">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
              <Zap size={18} className="text-yellow-500" /> 로봇 상태
            </h3>
          ...
          <div className="space-y-6">
             ...
             <div>
               <div className="flex justify-between text-sm mb-1">
                 ...
                 <span className={`font-bold ${robotStatus.battery < 20 ? 'text-red-600' : 'text-green-600'}`}>
        ...
        <section className="bg-white rounded-lg border border-gray-200 shadow-sm p-5 space-y-5">
          ...
          {/* (코드 줄임을 위해 생략했지만, 기존 RobotContext.jsx와 동일한 로직 유지) */}
           <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-100">
            <div className="flex justify-between items-center mb-3">
              ...
              {isVoiceCloned ? (
                <span className="text-[10px] bg-indigo-200 text-indigo-800 px-2 py-0.5 rounded-full font-bold">학습 완료</span>
              ) : (
                <button onClick={trainVoice} className="text-xs bg-white border border-indigo-200 px-2 py-1 rounded text-indigo-700 hover:bg-indigo-100 transition-colors">
                  학습 시작
                </button>
              )}
            </div>
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => isVoiceCloned && setUseClonedVoice(!useClonedVoice)}>
              <div className={`w-10 h-5 rounded-full relative transition-colors ${useClonedVoice ? 'bg-indigo-600' : 'bg-gray-300'}`}>
                <div className={`w-3 h-3 bg-white rounded-full absolute top-1 transition-all ${useClonedVoice ? 'left-6' : 'left-1'}`} />
              </div>
              <span className={`text-xs ${useClonedVoice ? 'text-indigo-700 font-medium' : 'text-gray-400'}`}>
                {useClonedVoice ? '내 목소리로 출력' : '기본 기계음 사용'}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <input 
              ...
              placeholder="로봇이 말할 내용 입력..."
              className="flex-1 text-sm border border-gray-300 rounded-lg px-3 py-2 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
            />
            <button 
              onClick={() => { sendTTS(ttsText); setTtsText(''); }}
              disabled={!ttsText.trim()}
              className="bg-gray-900 text-white px-3 rounded-lg hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
          ...
          >
            ...
            <span className="text-xs font-medium">
              {isRecording ? "녹음 중... (떼면 전송)" : "누르고 말하기 (무전기 모드)"}
            </span>
          </button>
        </section>
...
const ControlButton = ({ onClick, icon, label }) => (
  ...
  >
    ...
    {label && <span className="text-[10px] font-bold text-gray-400">{label}</span>}
  </button>
);
